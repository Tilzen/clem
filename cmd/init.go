...
const clemYAMLJiraTemplate = `# clem.yaml — Clementine team configuration (Jira coordination)
# Secrets go in secrets.sops.yaml (encrypted). Run: clem vault init
...
operator:
  ...
  jira_accounts: ["you@company.com"]     # trusted operator Jira accounts
...
func claudeSharedJiraTemplate() string {
	return `# CLAUDE.local.md — runtime contract for all {{project}} agents
# Generated by clem from CLAUDE.shared.md + CLAUDE.<agentkey>.md. Do not hand-edit this file.
...
Coordination uses Jira project **{{coordination.jira.project}}** on **{{coordination.jira.site}}**
via the **jira-mcp** tools (mcp-atlassian). Install with: ` + "`" + coordination.MCPAtlassianInstallCmd() + "`" + `.
...
### T4 — Maintenance (reactive only)
Act only on observed health problems. **Do not manufacture maintenance work.**

## Task board protocol

Discover work each iteration with jira_search using JQL such as:

` + "`project = {{coordination.jira.project}} AND labels = {{channels.tasks}} AND assignee is EMPTY`" + `
...
Trusted operator GitHub logins: {{operator.github_logins}}
Trusted operator Jira accounts: {{operator.jira_accounts}}
...
Your prompt ends with ` + "`kill $PPID`" + ` — required and expected.
`
}
...
func init() {
	...
	initCmd.Flags().String("backend", "discord", "coordination backend template (discord | github | jira)")
}
...
func runInit(cmd *cobra.Command, args []string) error {
	...
	case "jira":
		clemYAML = clemYAMLJiraTemplate
		claudeShared = claudeSharedJiraTemplate()
	default:
		return fmt.Errorf("unknown --backend %q (valid: discord, github, jira)", backend)
	}
	...
	if backend == "github" {
		...
		fmt.Printf("  3. %s on the host\n", coordination.MCPAtlassianInstallCmd())
